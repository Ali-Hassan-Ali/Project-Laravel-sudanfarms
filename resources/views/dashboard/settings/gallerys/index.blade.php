        ...
        <section class="content">
            ...
            <div class="box box-primary">
                ...
                <div class="box-header with-border">
                    ...
                    <h3 class="box-title" style="margin-bottom: 15px">@lang('dashboard.gallerys') <small>{{ $gallerys->total() }}</small></h3>

                    <form action="{{ route('dashboard.settings.gallerys.index') }}" method="get">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                ...
                                @if (auth()->user()->hasPermission('gallerys_create'))
                                    <a href="{{ route('dashboard.settings.gallerys.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> @lang('dashboard.add')</a>
                                @else
                            ...
                            <table class="table table-hover">
                                ...
                                @foreach ($gallerys as $index=>$gallery)
                                    <tr>
                                        ...
                                        <td>{{ $gallery->name }}</td>
                                        <td>
                                            <img src="{{ $gallery->image_path }}" width="50">
                                        </td>
                                        ...
                                        <td>
                                            @if (auth()->user()->hasPermission('gallerys_update'))
                                                <a href="{{ route('dashboard.settings.gallerys.edit', $gallery->id) }}" class="btn btn-info btn-sm"><i class="fa fa-edit"></i> @lang('dashboard.edit')</a>
                                            @else
                                            ...
                                            @if (auth()->user()->hasPermission('gallerys_delete'))
                                                <form action="{{ route('dashboard.settings.gallerys.destroy', $gallery->id) }}" method="post" style="display: inline-block">
                                                    {{ csrf_field() }}
                                                    {{ method_field('delete') }}
